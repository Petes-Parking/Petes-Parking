	...
	<div class="container">
		<div class="table-wrapper">
							...
							<td><c:choose>
									<c:when test="${sessionScope.user.userRole == 'Admin'}">
										<a class="nav-link active" aria-current="page"
											href="${pageContext.request.contextPath}/approveReq?id=${li.id}">Approve</a>
										<a class="nav-link active" aria-current="page"
											href="${pageContext.request.contextPath}/declineReq?id=${li.id}">Decline</a>
										<a class="nav-link active" aria-current="page"
											href="${pageContext.request.contextPath}/userCancel?id=${li.id}">Cancel</a>
										<c:choose>
											<c:when test="${li.paymentStatus!='Paid'}">
												<div>
													<sf:form method="post"
														action="${pageContext.request.contextPath}/paymentRequest"
														modelAttribute="form">
														<s:bind path="amount">
															<font color="red" style="font-size: 15px"><sf:errors
																	path="${status.expression}" /></font>
															<sf:input type="text" id="amount" class="form-control"
																path="amount" placeholder="Enter Amount" name="amount"
																style="width: 300px;" />
														</s:bind>
										...
										<c:choose>
											...
											<c:when test="${sessionScope.user.userRole == 'Student'}">
												<c:choose>
													<c:when test="${li.paymentStatus!='Paid'}">
														<a class="nav-link active" aria-current="page"
															href="${pageContext.request.contextPath}/payment?id=${li.id}">Pay
															Due ${paymentRequestDTO.amount} </a>
													</c:when>
													<c:otherwise>
														...
														<a class="nav-link active"
															...
															aria-current="page"
															href="${pageContext.request.contextPath}/payment?id=${li.id}">Paid
														</a>
													</c:otherwise>
												</c:choose>
											</c:when>
			...
			<c:choose>
				<c:when test="${sessionScope.user.userRole == 'Admin'}">
					<td>
						<form action="${pageContext.request.contextPath}/adminview"
							method="get">
